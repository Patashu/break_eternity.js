...
export default class Decimal {
  ...
  public static slog_critical(base: number, height: number): number {
	...
	var result = base_e_slog_values[Math.floor(height)]*(1-frac)
	+
	base_e_slog_values[Math.ceil(height)]*frac;
	return result;
  }
  
  public static tetrate_critical(base: number, height: number): number {
	var headers = [Math.E];
	var base_e_tetr_values = [
	1, //0.0
	...
	if (height > 10) { height = 10; }
	var frac = height-Math.floor(height);
	var result = base_e_tetr_values[Math.floor(height)]*(1-frac)
	+
	base_e_tetr_values[Math.ceil(height)]*frac;
	return result;
  }
